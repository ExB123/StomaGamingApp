...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // Важно для работы некоторых библиотек в Electron
...
ipcMain$1.handle("warface-get-stats", async (_event, nickname) => {
  var _a;
  try {
    const { WFClient, WFSClient } = await import("./index-CGy4utuj.js").then((n) => n.i);
    const client = new WFClient();
    if (!client.user || typeof client.user.stat !== "function") {
      throw new Error("Метод user.stat не найден в библиотеке");
    }
    const rawData = await client.user.stat(nickname);
    return { success: true, data: rawData };
  } catch (error2) {
    console.error("Ошибка в Main Process при запросе Warface:", error2);
    return {
      success: false,
      error: error2.message || "Неизвестная ошибка API",
      details: ((_a = error2.response) == null ? void 0 : _a.data) || error2.toString()
    };
  }
});
